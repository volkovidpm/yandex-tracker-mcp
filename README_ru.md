...
- **`issue_add_attachment`**: Прикрепить локальный файл к задаче (загружается из файловой системы MCP-сервера, до 1024 МБ)
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `file_path` (строка, обязательно): Путь к файлу в файловой системе MCP-сервера
    - `filename` (строка, опционально): Имя для сохранения вложения (по умолчанию — базовое имя из `file_path`)
  - Возвращает созданное вложение

- **`issue_add_checklist_item`**: Добавить новый пункт в чек-лист задачи
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `text` (строка, обязательно): Текст пункта чек-листа
    - `checked` (логическое, опционально): Отметить пункт выполненным
    - `assignee` (строка, опционально): Логин или ID исполнителя
    - `deadline` (объект, опционально): `{ date, deadline_type }`, где `deadline_type` — `date` или `quarter`
  - Возвращает весь чек-лист после добавления

- **`issue_update_checklist_item`**: Изменить пункт чек-листа (меняются только переданные поля)
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `item_id` (строка, обязательно): ID пункта чек-листа (как возвращает `issue_get_checklist`)
    - `text`, `checked`, `assignee`, `deadline` (все опционально): Новые значения
  - Возвращает весь чек-лист после изменения

- **`issue_delete_checklist_item`**: Удалить один пункт чек-листа
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `item_id` (строка, обязательно): ID пункта чек-листа (как возвращает `issue_get_checklist`)
  - Возвращает оставшийся чек-лист

- **`issue_delete_all_checklist_items`**: Удалить все пункты чек-листа сразу
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
  - Возвращает (теперь пустой) чек-лист
...
- **`issues_find_by_local_field`**: Поиск задач в очереди по значению локального поля очереди — обход ограничения, когда поле не зарегистрировано как фильтр поиска в Трекере
  - Параметры:
    - `queue` (строка, обязательно): Ключ очереди для поиска, например `SUP`
    - `field_key` (строка, обязательно): Ключ локального поля для сравнения, например `organization` (см. `queue_get_fields`)
    - `values` (список строк, обязательно): Значения поля для сравнения — точное совпадение без учёта регистра
    - `extra_query` (строка, опционально): Дополнительное условие YQL через AND к фильтру по очереди, например `Resolution: unresolved()`
    - `max_pages` (целое число, опционально, по умолчанию: 50): Ограничение на количество страниц по 100 задач для сканирования
  - Возвращает `{matches: [...], pages_scanned, truncated}` — каждый матч содержит `key`, `summary`, `status`, `assignee`, `created_at`, `field_value`
  - **Зачем это нужно**: многие текстовые локальные поля очереди (например, поле «Клиент»/«Организация» в очереди поддержки) не зарегистрированы как фильтры поиска в Трекере. Запрос к ним через `issues_find`/`issues_count` в виде `<QUEUE>.<field_key>: "значение"` падает с HTTP 422 `Фильтр <field> не существует`, хотя поле видно и заполнено у каждой задачи. Этот инструмент постранично обходит задачи очереди через `issues_find` и сравнивает значение поля на стороне клиента
  - Медленнее настоящего фильтра (ограничено `max_pages`) — если в ответе `truncated: true`, увеличьте `max_pages`, чтобы просканировать дальше в глубину
  - Учитывает ограничения `TRACKER_LIMIT_QUEUES`
